-- ============================================
-- SAFE TEAMS MIGRATION
-- Handles NULL owner_id values safely
-- ============================================

-- Step 1: Backup old teams table
CREATE TABLE IF NOT EXISTS teams_backup_old AS
SELECT * FROM teams;

-- Step 2: Drop old teams table
DROP TABLE IF EXISTS teams CASCADE;

-- Step 3: Create new teams table with correct structure
-- NOTE: owner_id is temporarily nullable during migration
CREATE TABLE public.teams (
  id bigserial PRIMARY KEY,
  name text NOT NULL,
  slug text UNIQUE,
  owner_id uuid REFERENCES auth.users(id) ON DELETE CASCADE,  -- Temporarily nullable
  description text,
...
-- Step 4: Migrate old data with NULL handling
DO $$
DECLARE
  old_team RECORD;
  new_slug text;
...
BEGIN
  -- Get the first user in the system as fallback owner
  SELECT id INTO default_owner_id FROM auth.users LIMIT 1;

  IF default_owner_id IS NULL THEN
    RAISE EXCEPTION 'No users found in database. Cannot migrate teams without at least one user.';
  END IF;

  -- Migrate each old team
  FOR old_team IN SELECT * FROM teams_backup_old LOOP
    ...
    VALUES (
      ...
      old_team.name,
      new_slug,
      COALESCE(old_team.created_by, default_owner_id),  -- Use default if NULL
      old_team.created_at,
      old_team.updated_at
    );

    IF old_team.created_by IS NULL THEN
      RAISE NOTICE '⚠️  Team "%" had NULL owner, assigned to first user', old_team.name;
    ELSE
      RAISE NOTICE '✓ Migrated team: %', old_team.name;
    END IF;
  END LOOP;
...
-- Step 5: Now make owner_id NOT NULL
...
-- Step 8: Create team members from migrated teams
DO $$
DECLARE
  team RECORD;
BEGIN
  FOR team IN SELECT * FROM teams LOOP
    ...
    VALUES (team.id, team.owner_id, 'owner', team.owner_id)
...
BEGIN
  ...
  RAISE NOTICE '  ✓ All tables and policies created';
  RAISE NOTICE '';
