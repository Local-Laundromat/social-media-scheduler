...
/**
 * Get OpenAI client for user - tries user's key first, then falls back to system key
 */
async function getOpenAIClient(userId) {
  let apiKey = process.env.OPENAI_API_KEY || null;
  let source = 'system';

  // Try to get user's personal OpenAI key
  if (userId) {
  ...
  if (!apiKey || apiKey === 'not-configured') {
    return { client: null, source: null, error: 'OpenAI API key not configured' };
  }

  const client = new OpenAI({ apiKey });
  return { client, source, error: null };
}
...
router.post('/generate-caption', authenticateToken, async (req, res) => {
  ...
  if (!openai) {
    return res.json({
      success: false,
      error: error || 'OpenAI API key not configured. Please add your OpenAI API key in Settings.'
    });
  }
